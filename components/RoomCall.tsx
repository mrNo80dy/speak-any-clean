...
export default function RoomPage() {
  ...
  // Manual text caption submit (uses same translation path as STT)
  ...
  const simulateDebugCaption = () => {
    ...
    // Simulate a local caption (You)
    pushMessage({
      fromId: clientId,
      fromName: "You",
      originalLang: "en-US",
      translatedLang: target,
      originalText: "Hello from debug (you).",
      translatedText: pickText(
        "Hello from debug (you).",
        "Olá do debug (você)."
      ),
      isLocal: true,
    });

    // Simulate a remote caption
    const remoteId = "remote-debug";
    pushMessage({
      fromId: remoteId,
      fromName: "Joice (debug)",
      originalLang: "pt-BR",
      translatedLang: target,
      originalText: "Olá do debug (remoto).",
      translatedText: pickText(
        "Hello from debug (remote).",
        "Olá do debug (remoto)."
      ),
      isLocal: false,
    });
      ...
      <div className="relative h-full w-full">
        ...
        <header className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between gap-2 flex-wrap px-4 py-2 bg-gradient-to-b from-black/70 to-transparent">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Debug: simulate captions (dev only) */}
            {process.env.NODE_ENV !== "production" && (
              <button
                onClick={simulateDebugCaption}
                className={`${pillBase} bg-purple-700 text-white border-purple-500`}
              >
                Test CC
              </button>
            )}
