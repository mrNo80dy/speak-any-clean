...
function FullBleedVideo({
  ...
  // Viewer-based framing:
  // - Desktop: contain (prevents “giant face” when viewing a phone portrait stream)
  // - Mobile portrait: cover (feels like a selfie/fullscreen camera)
  // - PiP: contain (so you can actually see what the other person sees), with blurred background fill
  useEffect(() => {
    ...
    const update = () => {
      ...
      setFit(isMobileUa && portrait ? "cover" : "contain");
    };
  ...
  useEffect(() => {
    ...
    // Clone for blurred background layer (prevents harsh letterboxing).
